...
1. **Layout cleanup removes live listeners** - Conversation subscriptions and RevenueCat customer-info listener are removed on effect cleanup.
2. **Firebase sign out is executed** - Auth session is revoked.
3. **Local stores are cleared/reset** - Recipes, messages, shopping, usage, and temporary Pro showcase state are wiped.
4. **Auth store resets in-memory user state** - App returns to signed-out UX.

The RevenueCat SDK also exposes `logout()` in the service layer for explicit identity clearing, but the current app flow relies on listener cleanup + Firebase sign-out + state reset.
...
| **Free Trial** | - | Intro offer configured in App Store Connect |
